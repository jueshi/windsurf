...
class StockDataManager:
    ...
    def initial_download(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Download error for {ticker}: {str(download_error)}")
                return None
            
            # Validate downloaded data
            if stock_data.empty:
                logging.warning(f"No data downloaded for {ticker} (empty DataFrame returned)")
                return None
    ...
    def update_data(self, ticker: str, force_download: bool = False) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # If force_download, skip existing data and download new
            if force_download:
...
def main():
    ...
    # stock_manager.process_stock_data(tickers=index_tickers,force_download=False)
