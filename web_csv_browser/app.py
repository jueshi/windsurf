...
@app.route('/api/columns/numeric', methods=['POST'])
def get_numeric_columns():
    ...
    try:
        ...
        numeric_cols = core.get_numeric_varying_columns(df)
        return jsonify({"numeric_columns": numeric_cols})
    except Exception as e:
        return jsonify({"error": str(e)})

@app.route('/api/plot/correlation', methods=['POST'])
def get_correlation_plots():
    ...
    try:
        ...
        corr_df, data_df = core.get_top_correlated_columns(df, target_col, top_n)
        plots = core.generate_correlation_plots_base64(data_df, target_col, corr_df)
