...
<div style="background:#e3f2fd;border-left:4px solid #1565c0;padding:12px 16px;margin:20px 0;line-height:1.5;">
<strong>Technical Charts Attached</strong><br>
Open the attached HTML file to view live daily, weekly, and monthly charts from StockCharts.com for all discussed tickers.
</div>
...
def _send_via_gmail_smtp(subject, body, recipient, attachments=None):
    ...
    msg_alt.attach(MIMEText("See HTML version of this email.", "plain", "utf-8"))
    ...
    if attachments:
        for filepath in attachments:
            try:
                ...
                msg.attach(part)
                logging.info(f"Attached: {filename}")
            except Exception as e:
                logging.warning(f"Failed to attach {filepath}: {e}")

    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
        server.login(sender, app_password)
        server.send_message(msg)

    logging.info(f"Email sent to {recipient} via Gmail SMTP ({len(attachments or [])} attachments)")
...
def main():
    ...
    # Generate live StockCharts.com HTML gallery for all discussed tickers
    logging.info("Generating StockCharts.com gallery for discussed tickers...")
    ...
    charts_dir = os.path.join(manager.plot_save_path, "email_charts")
    os.makedirs(charts_dir, exist_ok=True)
    charts_html_path = os.path.join(charts_dir, f"sector_charts_{datetime.now().strftime('%Y%m%d')}.html")
    _generate_stockcharts_gallery(all_discussed_tickers, charts_html_path, holdings_analysis)
    logging.info(f"Generated StockCharts gallery: {charts_html_path} ({len(all_discussed_tickers)} tickers)")
    ...
    if args.dry_run:
        print(body)
        logging.info(f"Dry run complete. Charts gallery: {charts_html_path}")
        return

    # Send email with charts HTML as attachment
    try:
        _send_via_gmail_smtp(subject, body, args.to, attachments=[charts_html_path])
    except Exception as e:
