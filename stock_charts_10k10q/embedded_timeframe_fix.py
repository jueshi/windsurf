"""
Embedded timeframe chart implementation for daily/weekly/monthly charts.
This module provides functionality to display stock data in all timeframes simultaneously
using embedded Plotly charts in Tkinter frames.
"""
...
def apply_embedded_timeframe_fix(app):
    """
    Apply the embedded timeframe chart fix to add daily/weekly/monthly chart functionality.
    
    Args:
        app: The GUI application instance
    """
    logging.info("Applying embedded timeframe chart fix...")
    
    # Add debug logging
    print("DEBUG: Starting embedded timeframe chart fix application")
    print(f"DEBUG: App type: {type(app)}")
    print(f"DEBUG: App has chart_notebook: {hasattr(app, 'chart_notebook')}")
    if hasattr(app, 'chart_notebook'):
        print(f"DEBUG: chart_notebook exists: {app.chart_notebook.winfo_exists()}")
    print(f"DEBUG: App has manager: {hasattr(app, 'manager')}")
    print(f"DEBUG: App has root: {hasattr(app, 'root')}")
    ...
    def add_timeframe_tab():
        """Add the timeframe chart tab to the notebook"""
        print("DEBUG: Inside add_timeframe_tab function")
        if not hasattr(app, 'chart_notebook'):
            print("DEBUG: app does not have chart_notebook attribute")
            logging.error("Cannot add timeframe tab: chart_notebook attribute not found")
            return False
        
        if not app.chart_notebook.winfo_exists():
            print("DEBUG: chart_notebook widget does not exist")
        ...
        try:
            ...
            daily_label = ttk.Label(app.daily_chart_frame, 
                                   text="Select a ticker to display daily chart")
            ...
            # Add a label for instructions
            weekly_label = ttk.Label(app.weekly_chart_frame, 
                                    text="Select a ticker to display weekly chart")
            ...
            # Add a label for instructions
            monthly_label = ttk.Label(app.monthly_chart_frame, 
                                     text="Select a ticker to display monthly chart")
    ...
    def generate_all_timeframe_charts(self, ticker):
        ...
        try:
            ...
            if df is None or df.empty:
                ...
                messagebox.showwarning("No Data", f"No data available for {ticker}. Please download data first.")
                return False
            
            # Generate each timeframe chart
            daily_success = self._generate_embedded_timeframe_chart(ticker, "Daily", self.daily_chart_frame, df)
            weekly_success = self._generate_embedded_timeframe_chart(ticker, "Weekly", self.weekly_chart_frame, df)
            monthly_success = self._generate_embedded_timeframe_chart(ticker, "Monthly", self.monthly_chart_frame, df)
    ...
    # Add the embedded timeframe chart generation method
    def generate_embedded_timeframe_chart(self, ticker, timeframe, chart_frame, df):
        """Generate an embedded chart for the specified ticker and timeframe"""
        try:
            logging.info(f"Generating embedded {timeframe} chart for {ticker}")
            ...
            # Save to temporary HTML file for browser viewing
            ...
            if chart_frame and chart_frame.winfo_exists():
                try:
                    ...
                    # Add a label with the chart title
                    ttk.Label(btn_frame, 
                             text=f"{ticker} {timeframe} Chart",
                             font=("Helvetica", 10, "bold")).pack(side=tk.LEFT, padx=5)
                    
                    # Add button to open in browser
                    ttk.Button(btn_frame, text="Open in Browser", 
                              command=lambda path=html_path: webbrowser.open(f"file:///{path}")).pack(side=tk.RIGHT, padx=5)
                    
                    # Create a static image of the chart for display in the frame
                    img_bytes = fig.to_image(format="png", width=800, height=400)
                    
                    # Convert bytes to PhotoImage
                    ...
                    logging.error(f"Error displaying {timeframe} chart: {str(e)}")
            ...
            logging.error(f"Error generating embedded timeframe chart: {str(e)}")
            if hasattr(self, 'status_var'):
                self.status_var.set(f"Error: {str(e)}")
            return False
    ...
    # Apply the fixes
    try:
        print("DEBUG: Starting to apply embedded timeframe fixes")
        
        # Add the timeframe tab
        tab_added = add_timeframe_tab()
        print(f"DEBUG: Timeframe tab added: {tab_added}")
        
        # Add the timeframe chart generation methods
        print("DEBUG: Adding chart generation methods to app")
        app._generate_all_timeframe_charts = types.MethodType(generate_all_timeframe_charts, app)
        app._generate_embedded_timeframe_chart = types.MethodType(generate_embedded_timeframe_chart, app)
        print("DEBUG: Chart generation methods added successfully")
        
        # Update the tab change handler
        print("DEBUG: Updating tab change handler")
        if hasattr(app, '_on_tab_changed'):
            ...
            print("DEBUG: Tab change handler updated successfully")
        else:
            print("DEBUG: App does not have _on_tab_changed method")
            # Try to find alternative tab change handler
            if hasattr(app, 'on_tab_changed'):
                print("DEBUG: Found on_tab_changed method instead")
                original_on_tab_changed = app.on_tab_changed
                app.on_tab_changed = types.MethodType(on_tab_changed, app)
                print("DEBUG: Alternative tab change handler updated successfully")
            else:
                print("DEBUG: No tab change handler found")
                # Add a direct binding to the notebook
                if hasattr(app, 'chart_notebook') and app.chart_notebook.winfo_exists():
                    print("DEBUG: Adding direct binding to chart_notebook")
                    app.chart_notebook.bind("<<NotebookTabChanged>>", lambda e: on_tab_changed(app, e))
                    print("DEBUG: Direct binding added successfully")
        ...
        logging.info("Embedded timeframe chart fix applied successfully")
        return True
    except Exception as e:
        logging.error(f"Error applying embedded timeframe chart fix: {str(e)}")
        return False
