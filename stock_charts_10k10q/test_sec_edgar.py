...
if __name__ == "__main__":
    main()
                        ...
                        logging.info(f"Successfully extracted text ({len(text)} characters). Preview:\n{text_preview}")
                    else:
                        logging.error(f"Failed to extract text from {filing_url}")
                else:
                    logging.error(f"Failed to retrieve {filing_type} URL for {ticker}")
            except Exception as e:
                logging.error(f"Error processing {filing_type} for {ticker}: {e}")
                traceback.print_exc()
            
            logging.info("-" * 80)

if __name__ == "__main__":
    test_sec_edgar_url_retrieval()
